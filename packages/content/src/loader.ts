...
import type { MaterialDef, MaterialProperties, ModelDefinition, SkeletonDef, Recipe, StructureDef, LoreFragment, ItemTemplate, NpcTemplate, ResourceNodeTemplate, ConceptVerbEntry, GameConfig, TileLayout, WeaponActionDef, VerbDef } from "./types.ts";
...
export async function loadContentStore(
  ...
  const [materialsRaw, modelsRaw, skeletonsRaw, recipesRaw, structuresRaw, loreRaw, itemTemplatesRaw, nodeTemplatesRaw, npcTemplatesRaw, conceptVerbRaw, weaponActionsRaw, verbsRaw] = await Promise.all([
