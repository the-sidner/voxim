...
const TOOLS: { mode: ToolMode; label: string; key: string }[] = [
  { mode: "paint", label: "Paint [P]", key: "P" },
  { mode: "erase", label: "Erase [E]", key: "E" },
  { mode: "fill",  label: "Fill [F]",  key: "F" },
];

const BTN: preact.JSX.CSSProperties = {
  padding: "4px 10px", border: "1px solid #555", borderRadius: 3,
  cursor: "pointer", background: "#2a2a2a", color: "#ccc", fontFamily: "monospace",
  fontSize: 12,
};
const BTN_ACTIVE: preact.JSX.CSSProperties = { ...BTN, background: "#4a7c3f", color: "#fff", borderColor: "#6aac5f" };
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "6px 8px", background: "#1e1e1e", borderBottom: "1px solid #333", flexShrink: 0 }}>
      {TOOLS.map((t) => (
        <button
          key={t.mode}
          style={tool === t.mode ? BTN_ACTIVE : BTN}
