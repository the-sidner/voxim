...
export class DodgeSystem implements System {
  ...
  run(world: World, _events: EventEmitter, _dt: number): void {
    ...
    for (const { entityId, inputState, combatState } of world.query(InputState, CombatState)) {
      const stagger = decrementCooldown(combatState.staggerTicksRemaining);
      const iFrames = decrementCooldown(combatState.iFrameTicksRemaining);
      const dodgeCd = decrementCooldown(combatState.dodgeCooldownTicks);
